...
@Component({
  selector: 'app-approve-rewards-and-recognization',
  templateUrl: './approve-rewards-and-recognization.component.html',
  styleUrls: ['./approve-rewards-and-recognization.component.css']
})
export class ApproveRewardsAndRecognizationComponent implements OnInit {
  constructor(private formBuilder: FormBuilder, private spinner: NgxSpinnerService,
    public crudOperationsService: CrudOperationsService, private notification: NotifierService,
    public datePipe: DatePipe) { }

  public headers: any = ['Department Name', 'Employee Details', 'Type', 'Name', 'Amount', 'Remarks', 'HOD Status', 'HOD Remarks','MGMT Status','MGMT Remarks', 'Actions'];
  public companyId: any = Number(sessionStorage.getItem('companyId'));
  public employeeId: any = Number(sessionStorage.getItem('empId'));
  public headCheck: any = sessionStorage.getItem('role')?.includes('HEAD');
  public designation: any = sessionStorage.getItem('designation');
  public rrsList: any = [];
  public statusList: any = ['Approve', 'Reject'];
  ...
  getRRsList() {
    ...
    this.crudOperationsService.getList(api).subscribe((data: any) => {
      this.spinner.hide();
      this.rrsOutList = data.data.content;
      if(this.designation == 'Management' || this.designation == 'MANAGEMENT'){
        this.rrsList = this.rrsOutList
      }else{
        for(let i=0;i<this.rrsOutList.length;i++){
          for(let j=0;j<this.rrsOutList[i].empDetails.length;j++){
            if(this.rrsOutList[i].empDetails[j].employeeId == this.employeeId){
              this.rrsList.push(this.rrsOutList[i]);
            }
          }
        }
      }
  ...
  loadFormData(data: any) {
    ...
    this.form_['hodStatus'].patchValue(data.hodStatus);
    this.form_['hodRemarks'].patchValue(data.hodRemarks);
    this.form_['mgmtStatus'].patchValue(data.mgmtStatus);
    this.form_['mgmtRemarks'].patchValue(data.mgmtRemarks);
  ...
  getFormData(): any {
    ...
    let object = {
      ...
      "hodStatus": this.rrsForm.value.hodStatus,
      "hodRemarks": this.rrsForm.value.hodRemarks,
      "mgmtStatus": this.rrsForm.value.mgmtStatus,
      "mgmtRemarks": this.rrsForm.value.mgmtRemarks,
  ...
  public getDepartmentheadsDetails(): void {
    ...
    this.crudOperationsService.getList('employee/employees/' + this._form.department)
      .subscribe((data: any) => {
