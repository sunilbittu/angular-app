...
@Component({
  selector: 'app-asset-status-report',
  templateUrl: './asset-status-report.component.html',
  styleUrls: ['./asset-status-report.component.css']
})
export class AssetStatusReportComponent implements OnInit {
  ...
  fetchAssetCategories() {
    this.assetCategories = [{statusId:1,statusName:"Issued"},{statusId:2,statusName:"NotIssued"},{statusId:3,statusName:"Damage"} ];
   ...
   generateAssetReport() {
    ...
    if (this.selectedCategoryIds.length>0) {
      ...
      console.log(empList,"selected of assets",this.selectedCategoryIds)
        let repObj = { "statusIds": this.selectedCategoryIds,  "employeeIds": empList };
        this.employeeObjForexport=repObj;
        let url='issuing-asset/assetstatus_report/'+this.companyId;
        console.log(repObj)
  ...
  exportTable(type: string) {
    ...
    this.spinner.show();
    var fileType = '';
    let fileName='asset-status-report';
    if (type == 'EXCEL') {
    ...
    let api: any = "reports/assetStatusReport/"+this.companyId +"/"+ type ;
