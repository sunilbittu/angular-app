...
export class DeletePayslipComponent implements OnInit {
  public years: any = [];
  public selectedYear = '';
  ...
  applyFilter() {
    ...
    var filterJson = {
     ...
     if(this.role!='ROLE_EMPLOYEE'){
      this.employeeService.getEmployeesByFilterForPayrollGeneratedMultipleMonth(filterJson,this.selectedItems)
      .subscribe((data: any) => {
   ...
   const eItems = this.employeeList.filter((item: any) => item.selected === true);
    ...
    if (eItems.length > 0 && !flag) {
     // this.submitProcessing = true;
      ...
      //     })
      // } else {
        this.crudOperationsService.deletePayslip(empList,this.selectedItems)
          .subscribe((data: any) => {
            this.employeeList = data.data;
          //  this.submitProcessing1 = false;
            this.notification.notify('success', data.message);
