...
export class RepairRequestComponent implements OnInit {
  ...
  getBarcodeOfItems(assetId: any) {
    let api = 'issuing-asset/getNotIssuesAndDamageBarcodeOfItems?assetId=' + assetId;
    this.crudOperationsService.getList(api).subscribe((data: any) => {
  ...
  loadData(data: any) {
    ...
    this.repairForm_['repairDate'].patchValue(this.datePipe.transform(data.repairDate, 'dd-MM-yyyy'));
    this.repairForm_['description'].patchValue(data.description);
    this.selectedDate = new Date(data.repairDate);
  }
  edit(data: any) {
    this.readOnly = false;
    this.submitted = false;
  ...
  getFormData(): any {
    ...
    this.repairForm.value.repairDate = this.selectedDate;
