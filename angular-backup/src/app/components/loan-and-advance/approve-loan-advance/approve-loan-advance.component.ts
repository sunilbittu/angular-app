...
@Component({
  selector: 'app-approve-loan-advance',
  templateUrl: './approve-loan-advance.component.html',
  styleUrls: ['./approve-loan-advance.component.css']
})
export class ApproveLoanAdvanceComponent implements OnInit {
  ...
  public years: any = [];
  // public headers2: any = ["Employee","Type", "Loan/Advance Name", "Applied Date", "Budget Limit", "Amount Required", "EMI Calculated field", "Tenure Months", "Start Recovery From", "Status", "Purpose", "Docs","Approve Status", "Actions"];
  // public headers3: any = ["Employee","Type", "Loan/Advance Name", "Applied Date", "Budget Limit", "Amount Required", "EMI Calculated field", "Tenure Months", "Start Recovery From", "Status", "Purpose", "Docs","Approve Status"];
  ...
  public selectedApprovedDate: any;
  public now = new Date();
  ...
  getAppliedLoanAndAdvance() {
    this.spinner.show();
    this.outLoansList = [];
    let api = this.api + "/getLoansById?employeeId=" + this.employeeId;
    ...
    let listApi = this.api + "/getLoansByCompanyId/?companyId=" + this.companyId;
    this.crudOperationsService.getList(listApi)
      .subscribe((data: any) => {
        // this.spinner.hide();
        this.loansList = data.data;

        for (var i = 0; i < this.loansList.length; i++) {
          //let reimbursement = this.reimbursementsList[i];
          let userId = sessionStorage.getItem("empId");
          //console.log("reimbursement value is ========= ",this.loansList[i]);
          let json = this.loansList[i];
          let workflowId = this.loansList[i].loanTypeId.approvalWorkflow.id;
          //let loanId = this.loansList[i].reimbursementId;
          ...
          console.log("Out loans is ======= ", this.outLoansList);
  ...
  modelShowEdit(employee: any) {
    ...
    let userId = sessionStorage.getItem("empId");
    let approvedCheckApi = 'apply_reimbursement/getApprovalLevel?workFlowId=' + employee.loanTypeId.approvalWorkflow.id + '&employeeId=' + userId;
    this.crudOperationsService.getList(approvedCheckApi)
      .subscribe((data: any) => {
        // this.spinner.hide();
        console.log("data is ========== ", data);
    ...
    this.loanForm.controls['appliedDate'].patchValue(employee.appliedDate);
    ...
    this.loanForm.controls['status'].patchValue('Active');
    this.loanAmount = employee.loanAmount;
    this.installmentMonths = employee.installmentMonths;
    ...
    this.loanForm.controls['approvestatus'].enable();
    this.loanForm.controls['remarks'].enable();
    this.loanForm.controls['approvedAmount'].enable();
    this.loanForm.controls['approvedDate'].enable();
  ...
  changeTennureMonths(ten: any) {
    ...
    if (amount) {
      ...
      this.loanForm.controls['emiCalculatedField'].patchValue(emiAmount);
    }
  }
  ...
  onApprovedDateValueChange(event: any) {
    this.selectedApprovedDate = new Date(event);
  }
  ...
  update(formData: any, api: string) {
    ...
    this.approveLoanDTO = { 
      "status": this.loanForm.value.approvestatus, 
      "approverName": this.approvedBy, 
      "approvedAmount": this.loanForm.value.approvedAmount, 
      "approvedDate": this.loanForm.value.approvedDate, 
      "remarks": this.loanForm.value.remarks,
      "approverId": this.employeeId 
    };
    const approveFormData = new FormData();
    approveFormData.append('approveLoanDTO', JSON.stringify(this.approveLoanDTO));
    approveFormData.append('loanId', this.id);
    this.submitProcessing = true;
    this.crudOperationsService.create(approveFormData, this.api + '/approveLoan')
      .subscribe((data: any) => {
  ...
  checkValue(approveReimbursements: any) {
    ...
    if (approveReimbursements != null) {
      if (approveReimbursements.some((approve: any) => approve.approverName === this.approvedBy)) {
        // do something
        return true;
      } else {
        return false;
  ...
  public isAmtReqInValid = false;
  changeLimit() {
    this.isAmtReqInValid = this.loanForm.value.loanLimit < 1000;
  }
}
