...
export class InvoicesReportComponent implements OnInit {
  ...
  public headers2: any = ["Invoice No", "Client Name", "Candidate Name","Position Title","Recruiter", "Salary Offered", "Invoice Amount", "Invoice Sent Date","Realized Date"];
  ...
  this.spinner.show();
    let api: any = "candidate/findAllInvoicesForReport?year="+this.branchModel+"&quater="+this.departmentModel+"&type="+this.invoiceType;
    this.crudOperationsService.getList(api).subscribe((data: any) => {
      this.spinner.hide();
