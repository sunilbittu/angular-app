...
@Component({
  selector: 'app-employee-rejoin',
  templateUrl: './employee-rejoin.component.html',
  styleUrls: ['./employee-rejoin.component.css']
})
export class EmployeeRejoinComponent implements OnInit {
  ...
  public form = this.fb.group({
    rejoinDate: ["", Validators.required],
  });
  ...
  fetchEmployees() {
    ...
    let api: any = "leavemasterindividual/employee_dep_list_rejoin/" + this.departmentModel + "/" + this.companyId;
  ...
  editPromotion(data: any) {
    ...
    this.form.reset();
    this.rejoinDate = data.rejoinDate;
  ...
  relieveEmployee(data: any) {
    ...
    if (this.form.valid) {
      this.rejoinObject =
      {
        "relieveDate": this.form.value.rejoinDate
      }
      let api: any = "leavemasterindividual/rejoinEmployee/" + data.employeeId + "/" + this.empResgId;
      this.crudOperationsService.create(this.rejoinObject, api).subscribe((resp: any) => {
  ...
  onHiringStartDateValueChange(event: any) {
    this.rejoinDate = new Date(event);
  }
