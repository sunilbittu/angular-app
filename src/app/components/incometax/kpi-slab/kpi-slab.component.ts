...
@Component({
  selector: 'app-kpi-slab',
  templateUrl: './kpi-slab.component.html',
  styleUrls: ['./kpi-slab.component.css']
})
export class KPISlabComponent implements OnInit {

  public incometaxHeaders: any = ["KPI Name", "From", "To", "Incentive", "Action"];
  public slabData: any;
  public submitText: string = '';
  public searchModel: string = '';
  public kpiTypeModel: string = '';
  ...
  form = this.fb.group({
    kpiTypeModel:["",Validators.required],
  ...
  ngOnInit(): void {
    ...
    let api = "insurancecompany/list?companyId=" + this.companyId + '&remarks=' + 'kpi' +'&search=' + this.searchModel+"&page=&size=";
    this.crudOperationsService.getList(api).subscribe((data: any) => {
      //this.spinner.hide();
      //console.log(data, "===data");
      this.kpiList = data.data.content;
    })
  }
  ...
  fetchIncomeTaxSlabs() {
    this.spinner.show();
    this.crudOperationsService.getList("payslipslab/slobList/kpi1").subscribe((res: any) => {
      this.spinner.hide();
      this.toggleLoader = true;
  ...
  totaleis(){
   ...
   //alert(input);
   this.form.value.employeeContribution = 0;
   this.form.value.totalContribution = (this.employerContribution + this.employeeContribution).toFixed(2);
  }
  check() {
    if (this.submitText == 'Save') {
      this.isTaxSlabExist = false;
      let api = 'payslipslab/validate?fromSalary=' + this.form.value.fromSalary + '&toSalary=' + this.form.value.toSalary+ '&slabType=' + "socso";
  ...
  submit() {
    ...
    if (!this.isTaxSlabExist) {
     ...
     // const financialYear = { 'financialYearId': this.financialYearModel };
      ...
      this.form.value.slabType = this.kpiTypeModel;
      if (this.submitText == 'Save') {
  ...
  deleteAllSlab() {
    this.crudOperationsService.delete("payslipslab/deleteAll/" + "socso").subscribe((data: any) => {
      this.fetchIncomeTaxSlabs();
      this.deleteAlert = true;
      setTimeout(() => {
        this.deleteAlert = false;
