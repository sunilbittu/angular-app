...
@Component({
  selector: 'app-eis-slab',
  templateUrl: './eis-slab.component.html',
  styleUrls: ['./eis-slab.component.css']
})
export class EISSlabComponent implements OnInit {
  ...
  fetchIncomeTaxSlabs() {
    this.spinner.show();
    this.crudOperationsService.getList("payslipslab/slobList/eis").subscribe((res: any) => {
      this.spinner.hide();
      this.toggleLoader = true;
  ...
  sortColumn(){
    console.log("eis inside the sortColumn method!");
  ...
  check() {
    if (this.submitText == 'Save') {
      ...
      let api = 'payslipslab/validate?fromSalary=' + this.form.value.fromSalary + '&toSalary=' + this.form.value.toSalary+ '&slabType=' + "eis";
  ...
  deleteAllSlab() {
    this.crudOperationsService.delete("payslipslab/deleteAll/" + "eis").subscribe((data: any) => {
      this.fetchIncomeTaxSlabs();
      this.deleteAlert = true;
      setTimeout(() => {
        this.deleteAlert = false;
  ...
  exportExcel(){
    ...
    let fileType = 'EISSlabMaster.xls';
    let api: any = "payslipslab/slabExport/eis";
