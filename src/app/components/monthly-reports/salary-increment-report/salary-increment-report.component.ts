...
@Component({
  selector: 'app-salary-increment-report',
  templateUrl: './salary-increment-report.component.html',
  styleUrls: ['./salary-increment-report.component.css']
})
export class SalaryIncrementReportComponent implements OnInit {
  ...
  public headers: any = ["Employee Id","Employee Name", "Old CTC", "New CTC","Increment Date","% of Increment"];
  ...
  generatedexpenseReport() {
    ...
    if (this.startDate == '' && this.endDate == '') {
      this.monthAndYearValidationText = 'Start and End Date ' + this.errorMsg;
      flag = true;
    } else {
      if (this.endDate == '') {
    ...
    if (eItems.length > 0 && !flag) {
      ...
      for (let i = 0; i < this.employeeList.length; i++) {
        ...
        let repObj = { "employeeIds": empList };
        this.employeeObjForexport=repObj;
        let url='salary-increment/salary_increment_report/'+this.companyId+'/'+this.startDate+'/'+this.endDate;
  ...
  exportTable(type: string){
    ...
    var fileType = '';
    if (type == 'EXCEL') {
      fileType = 'EmployeeSalaryIncrementReport.xls';
      let api: any = "reports/salaryIncrementReport/exportExcel/"+this.companyId+"/"+this.startDate+'/'+this.endDate;
      this.crudOperationsService.exportExcelReport2(api,fileType,this.employeeObjForexport)
    }
    else {
      fileType = 'EmployeeSalaryIncrementReport.pdf';
      let api: any = "reports/salaryIncrementReport/exportPDF/"+this.companyId+"/"+this.startDate+'/'+this.endDate;
