...
@Component({
  selector: 'app-employee-l-loan-advances-reports',
  templateUrl: './employee-l-loan-advances-reports.component.html',
  styleUrls: ['./employee-l-loan-advances-reports.component.css']
})
export class EmployeeLLoanAdvancesReportsComponent implements OnInit {

  
  public years: any = [];
  public selectedYear = '';
  public months: any = ["January", "February", "March", "April", "May", "June","July", "August", "September", "October", "November", "December"];
  public headers: any = ["Employee Code","Employee Name", "Branch", "Department","Type","Name","Amount","Recovered","Balance","Status"];
  ...
  generatedexpenseReport() {
    ...
    if (eItems.length > 0 && !flag) {
      ...
      for (let i = 0; i < this.employeeList.length; i++) {
        ...
        let repObj = { "year": sessionStorage.getItem("selectedPayslipYear"), "month": sessionStorage.getItem("selectedPayslipMonth"),"statusIds": this.selectedCategoryIds, "employeeIds": empList };
        this.employeeObjForexport=repObj;
        let url='employeeloanadvance/loan_advance_report/'+this.companyId;
        this.crudOperationsService.create(repObj,url).subscribe((resp:any)=>{
          ...
          if(resp.data != ''){
            ...
            this.totalAmount= this.expenseReportList[0].totalAmount;
            this.totalRecovered= this.expenseReportList[0].totalRecovered;
            this.totalBalance=this.expenseReportList[0].totalBalance;
          }
        });
  ...
  exportTable(type: string){
    ...
    var fileType = '';
    if (type == 'EXCEL') {
      fileType = 'EmployeeLoanAdvanceReport.xls';
      let api: any = "reports/loanReport/exportExcel/"+this.companyId;
      this.crudOperationsService.exportExcelReport2(api,fileType,this.employeeObjForexport)
    }
    else {
      fileType = 'EmployeeLoanAdvanceReport.pdf';
      let api: any = "reports/loanReport/exportPDF/"+this.companyId;
