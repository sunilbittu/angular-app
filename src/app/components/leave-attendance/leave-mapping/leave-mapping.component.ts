...
export class LeaveMappingComponent implements OnInit {
  ...
  public destinationLeave: any = '';
  public sourceDept: any = '';
  public destinationYear: any = '';
  public sourceProject: any = '';
  public leaveYear: any = ['2026','2025','2024', '2023', '2022', '2021', '2020', '2019', '2018', '2017'];
  ...
  getEmployeeByLeaveTypeAndYear() {
    this.crudOperationsService.getList('leavemasterindividual/getEmployeeByLeaveTypeAndYear?companyId=' +
      this.companyId + '&leaveType=' + this.leaveType + '&year=' + this.year.toString()).subscribe((data: any) => {
  ...
  addLeft() {
    ...
    if (eItems.length > 0) {
      ...
      console.log("Destination List : ",this.destinationEmployeeList)
      console.log("List from DB : ",this.destinationEmployeeListFromDB)
      console.log("Selectd employees : ",this.employeeList)
      this.destinationEmployeeList.forEach((employee: any) => {
  ...
  clearDestination() {
    this.form_1['leave'].setValue('');
    this.form_1['year'].setValue('');
  }
  onClick() {
    this.isFormValuesSame = false;
  ...
  public noOfLeaves: any;
  public year: any;
  changeLeaveDestination(model: any) {
    this.leaveType = model.leaveTypeCode;
    this.noOfLeaves = model.numberOfLeave;
    // this.getEmployeeByLeaveTypeAndYear();
    this.searchDestination();
  }
  changeYearDestination(model: any) {
    this.year = model;
    // this.getEmployeeByLeaveTypeAndYear();
    this.searchDestination();
  }
  ...
  searchDestination() {
    ...
    let api: any = 'leavemasterindividual/getEmployeeByLeaveTypeAndYear?companyId=' +
      this.companyId + '&leaveType=' + this.leaveType + '&year=' + this.year.toString();
    this.crudOperationsService.getList(api)
      .subscribe((data: any) => {
  ...
  update() {
    ...
    let employeeIds = [];
    let leaveIds = [];
    console.log("des",this.destinationEmployeeList);
    if (this.destinationEmployeeList.length > 0) {
      for (let i = 0; i < this.destinationEmployeeList.length; i++) {
        if (this.destinationEmployeeList[i].externalId == null) {
          ...
          leaveIds.push(this.destinationEmployeeList[i].externalId);
        }
      }
    }
    if (employeeIds.length > 0 || leaveIds.length > 0) {
      this.spinner.show();
      var filterJson = {
        "leaveIds": leaveIds,
      ...
      let api = 'leavemasterindividual/employeeLeaveMappingUpdate';
      this.crudOperationsService.create(filterJson, api)
        .subscribe((data: any) => {
          if (data.data != null) {
            this.notification.notify('error', data.data);
          } else {
            this.notification.notify('success', 'Leave Mapped Successfully!');
            this.ngOnInit();
            this.onClick();
          }
          this.spinner.hide();
